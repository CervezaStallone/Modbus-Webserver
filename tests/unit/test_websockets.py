...
from modbus_app.consumers import AlarmConsumer, DashboardConsumer, DeviceConsumer

pytestmark = pytest.mark.skip(
    reason="WebSocket tests require full async setup with Redis"
)
...
class TestDashboardConsumer:
    ...
    async def test_dashboard_consumer_receives_register_update(self):
        ...
        await communicator.send_json_to(
            {"type": "register_update", "register_id": 1, "value": 25.5, "unit": "°C"}
        )
...
class TestDeviceConsumer:
    ...
    async def test_device_consumer_connect(self, device):
        ...
        communicator = WebsocketCommunicator(
            DeviceConsumer.as_asgi(), f"/ws/device/{device.id}/"
        )
    ...
    async def test_device_consumer_receives_device_update(self, device):
        ...
        await communicator.send_json_to(
            {"type": "device_update", "device_id": device.id, "status": "online"}
        )
...
class TestAlarmConsumer:
    ...
    async def test_alarm_consumer_connect(self):
        ...
        communicator = WebsocketCommunicator(AlarmConsumer.as_asgi(), "/ws/alarms/")
